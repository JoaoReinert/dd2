void main() {
  ...
  for (final controle in controleDePessoas.pessoasPeloMes.entries) {
    final mes = controle.key;
    final pessoas = controle.value;
...
class ControleDePessoas {
  final Map<Mes, List<Pessoa>> pessoasPeloMes = {};

  //cadastrar uma pessoa no sistema
  void cadastrarPessoa(Pessoa pessoa) {
    final mes = pessoa.mesNascimento;

    if (!pessoasPeloMes.containsKey(mes)) {
      pessoasPeloMes[mes] = [];
    }
    pessoasPeloMes[mes]!.add(pessoa);
  }

  Map<Mes, List<Pessoa>> get pessoaPorMes => pessoasPeloMes;
}
